...
#include <algorithm>
#include <cmath>using namespace std;
unsigned char convolution(int,int,double [][3],unsigned char[][258][1],double);
double getGaussian(double [][3]);int main(int argc, char *argv[]){	// Define file pointer and variables	FILE *file;	int BytesPerPixel;	int Size = 256;	// Check for proper syntax	if (argc < 3){		cout << "Syntax Error - Incorrect Parameter Usage:" << endl;		cout << "program_name input_image.raw output_image.raw [BytesPerPixel = 1] [Size = 256]" << endl;		return 0;	}	// Check if image is grayscale or color	if (argc < 4){		BytesPerPixel = 1; // default is grey image	}	else {		BytesPerPixel = atoi(argv[3]);		// Check if size is specified		if (argc >= 5){			Size = atoi(argv[4]);		}	}
    int height = Size;
    int width = Size;
...
unsigned char convolution(int i, int j, double kernel[][3], unsigned char image[][258][1],double sum){
    float pix = 0.0;
    for(int k=i; k<i+3; ++k){
        for(int l=j; l<j+3; ++l){
...
double getGaussian(double kernal[][3]){
    double sigma = 2.0;
    double sum=0;

    for(int i= -1; i<=1 ; ++i){
        for(int j= -1; j<=1; ++j){
            kernal[i+1][j+1] = exp(-(i*i + j*j)/(2*sigma * sigma))/(sqrt((2*M_PI * sigma * sigma)));
            sum+=kernal[i+1][j+1];
        }
    }
    return sum;
