...
unsigned char median(int i, int j, int channel){
	...
	for(int j=0; j<13; ++j){
        for(int i=13; i<13+width; ++i){
            ...
            Reimage[j][i][1] = Imagedata[13-j][i-13][1];
            Reimage[j][i][2] = Imagedata[13-j][i-13][2];
        }
	}
	cout<<"first"<<endl;
	for(int j=0; j<13; ++j){
        for(int i=13; i<13+width; ++i){
            ...
            Reimage[height+j+13][i][1] = Imagedata[height-j-2][i-13][1];
            Reimage[height+j+13][i][2] = Imagedata[height-j-2][i-13][2];
        }
	}
    cout<<"second"<<endl;
	for(int i=0; i<13; ++i){
        for(int j=13; j<13+height; ++j){
            ...
            Reimage[j][i][1] = Imagedata[j-13][13-i][1];
            Reimage[j][i][2] = Imagedata[j-13][13-i][2];
        }
	}
	...
	for(int i=0; i<13; ++i){
        for(int j=13; j<13+height; ++j){
            ...
            Reimage[j][width+13+i][1] = Imagedata[j-13][width-i-2][1];
            Reimage[j][width+13+i][2] = Imagedata[j-13][width-i-2][2];
        }
	}
	cout<<"continued"<<endl;
    ...
    for(int j=0; j<height; ++j){
        for(int i=0; i<width; ++i){
            ...
            Reimage[j+13][i+13][1] = Imagedata[j][i][1];
            Reimage[j+13][i+13][2] = Imagedata[j][i][2];
        }
    }
    ...
    if (!(file=fopen(argv[2],"wb"))) {		cout << "Cannot open file: " << argv[2] << endl;		exit(1);	}	fwrite(OutputImage, sizeof(unsigned char), (height)*(width)*3, file);	fclose(file);
